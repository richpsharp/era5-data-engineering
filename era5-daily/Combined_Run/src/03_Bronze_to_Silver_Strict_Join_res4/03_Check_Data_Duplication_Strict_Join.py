...
# MAGIC When running in the dev workspace, the notebook sets up the necessary catalog, schema, and table names. The full table name, `aer_era5_silver_strict_1950_to_present_dev_interpolation`, is constructed for querying the bronze-tier ERA5 climate data. This table contains climate records such as temperature and precipitation for various time steps and geolocations.
# MAGIC
...
if workspace_url == dev_workspace_url:
    ...
    schema = 'silver_dev'
    table = 'aer_era5_silver_strict_1950_to_present_dev_interpolation_res4'
    ...
    df_grid_index_lat_lon_duplicates = df_grid_index_lat_lon_duplicates.join(df, on=['grid_index', 'latitude', 'longitude', 'time'], how='inner') \
        .select("time", "latitude", "longitude", "count", "file_modified_in_s3", "source_file", 
                "Source_File_Path", "Ingest_Timestamp", "data_provider", "date_created","grid_index","country_strict","country_chip_wkb")

    # Save this to a Delta table
    df_grid_index_lat_lon_duplicates.write.format("delta").mode("overwrite") \
        .saveAsTable(f"{catalog}.{schema}.strict_join_time_lat_lon_grid_index_duplicates_with_details_res4")
    ...
    for var in variables:
        ...
        df_variable_duplicates = df_variable_duplicates.join(df, on=['latitude', 'longitude', 'time', var], how='inner') \
            .select("time", "latitude", "longitude", var, "count", "file_modified_in_s3", 
                    ...
                    "data_provider", "date_created","grid_index","country_strict","country_chip_wkb")
        
        # Save each variable's duplicates to a separate Delta table
        df_variable_duplicates.write.format("delta").mode("overwrite") \
            .saveAsTable(f"{catalog}.{schema}.strict_join_duplicates_{var}_res4") 
    ...
    table = 'aer_era5_silver_strict_1950_to_present_staging_interpolation_res4'

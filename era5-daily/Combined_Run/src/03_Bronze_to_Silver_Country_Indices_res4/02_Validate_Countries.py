...
if workspace_url == dev_workspace_url: 
    ...
    table_name = "`era5-daily-data`.silver_dev.esri_worldcountryboundaries_global_silver_res4"
    ...
    if spark._jsparkSession.catalog().tableExists(table_name):
     ...
     # Define the path or table name for the Delta table
    table_name = "`era5-daily-data`.silver_staging.esri_worldcountryboundaries_global_silver_res4"
...
df_raw = spark.read.table("countries_raw_res4") 
...
# MAGIC %sql select * from countries_raw_res4 where geom_0 is null
...
# MAGIC %sql select format_number(count(1), 0) as count from countries_raw_res4 where is_valid = false

# COMMAND ----------

# MAGIC %sql select geom_id, country, land_type from countries_raw_res4 where is_valid = false
...
# MAGIC %sql select distinct from_srid, count(1) as count from countries_raw_res4 group by from_srid

# COMMAND ----------

# MAGIC %sql select geom_id, country, land_type from countries_raw_res4 where from_srid = 0
...
# MAGIC %sql select geom_id, country, land_type from countries_raw_res4 where shape_area = 0 order by country
...
# MAGIC %sql select country, shape_area, is_valid from countries_raw_res4 where order by shape_area desc

# COMMAND ----------

# MAGIC %sql select country, shape_area, st_area(geom_0) as calc_area, is_valid from countries_raw_res4 where order by shape_area desc
...
# sql("drop table if exists countries_4326_res4")
if not spark.catalog.tableExists(f"{schema_name}.countries_4326_res4"):
   (
     spark.read.table("countries_raw_res4")
         ...
         .write
         .mode("overwrite")
         .saveAsTable("countries_4326_res4")
   )

df_4326 = spark.read.table("countries_4326_res4") 
...
# sql("drop table if exists countries_valid_res4")
if not spark.catalog.tableExists(f"{schema_name}.countries_valid_res4"):
  (
    spark.read.table("countries_4326_res4") 
      .withColumn(
        "geom_0",
        F.when(
      ...
      .write
      .mode("overwrite")
      .saveAsTable("countries_valid_res4")
  )

df_valid = spark.read.table("countries_valid_res4") 
...
# sql("drop table if exists countries_flat_res4")
if not spark.catalog.tableExists(f"{schema_name}.countries_flat_res4"):
  (
    spark.read.table("countries_valid_res4") 
      .withColumn(
        "geom_0",
      ...
      .write
      .mode("overwrite")
      .saveAsTable("countries_flat_res4")
  )

df_flat = spark.read.table("countries_flat_res4") 

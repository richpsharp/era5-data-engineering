...
df_flat = spark.table("countries_flat_res4")
...
def df_tessellate(
  ...
  if overwrite:
    ...
    sql("drop table if exists countries_h3_res4")  

  df_filter = (
    spark.table("countries_flat_res4")
      .filter(
  ...
  if do_display:
    _df = spark.table("countries_h3_res4").filter(F.expr(f"land_type = '{land_type}'"))
...
# MAGIC ALTER TABLE countries_h3_res4 CLUSTER BY (cellid);
# MAGIC OPTIMIZE countries_h3_res4;
# MAGIC DESCRIBE DETAIL countries_h3_res4;

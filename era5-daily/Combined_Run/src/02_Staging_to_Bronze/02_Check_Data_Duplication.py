...
# MAGIC When running in the dev workspace, the notebook sets up the necessary catalog, schema, and table names. The full table name, `aer_era5_bronze_1950_to_present_dev_interpolation`, is constructed for querying the bronze-tier ERA5 climate data. This table contains climate records such as temperature and precipitation for various time steps and geolocations.
# MAGIC
# MAGIC ### Data Quality Checks
# MAGIC The notebook performs crucial data quality checks, focusing on:
# MAGIC - **Duplicate Detection**: Identifies duplicate entries based on `latitude`, `longitude`, and `time` combinations.
...
# MAGIC - Identify duplicate `latitude`, `longitude`, and `time` entries.
...
if workspace_url == dev_workspace_url:
    ...
    # Identify duplicates for latitude and longitude columns within the same time step
    df_lat_lon_duplicates = df.groupBy('latitude', 'longitude', 'time') \
        .agg(count("*").alias("count")) \
        .filter(col("count") > 1)

    # Join back with the original DataFrame to get all columns
    df_lat_lon_duplicates = df_lat_lon_duplicates.join(df, on=['latitude', 'longitude', 'time'], how='inner') \
    ...
    df_lat_lon_duplicates.write.format("delta").mode("overwrite") \
        .saveAsTable(f"{catalog}.{schema}.time_lat_lon_duplicates_with_details")
    ...
    for var in variables:
        ...
        df_variable_duplicates.write.format("delta").mode("overwrite") \
            .saveAsTable(f"{catalog}.{schema}.duplicates_{var}")
